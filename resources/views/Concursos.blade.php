<x-layout>
    ...
    <div class="mt-8">
        <div class="flex justify-between items-center mb-4">
            ...
            <button id="inscribirBtn" class="bg-green-500 text-white px-4 py-2 rounded">+ Inscribir mi perro</button>
        </div>
                            ...
                            <td class="px-4 py-2 border-b" style="max-width: 200px;">
                                <div class="truncate" x-data="{ showTooltip: false }" @mouseenter="showTooltip = true"
                                    ...
                                    <div x-show="showTooltip"
                                        class="absolute bg-gray-700 text-white text-xs rounded py-1 px-4 z-10"
    ...
    <!-- Modal -->
    <form id="inscripcionForm" method="POST" action="{{ route('Concursos') }}">
        @csrf
                            ...
                            @foreach ($pruebas as $prueba)
                                ...
                                @foreach ($fechas as $fecha)
                                    <div class="flex items-center">
                                        <input id="fecha_{{ $loop->parent->index }}_{{ $loop->index }}"
                                            name="fechas[]" type="checkbox" value="{{ $fecha }}"
    ...
    <script>
        ...
        // Evento para enviar la inscripción
        document.getElementById('inscribirBtn').addEventListener('click', function() {
            const inscripciones = [];

            // Recolectar los datos de las inscripciones
            document.querySelectorAll('.inscripcion').forEach((inscripcion, index) => {
                const pruebaId = inscripcion.querySelector('#prueba').value;
                const perroId = inscripcion.querySelector('#perro').value;
                const fechas = [];
                inscripcion.querySelectorAll(`input[name="fechas_${index}[]"]:checked`).forEach(
                    checkbox => {
                        fechas.push(checkbox.value);
                    });
