<x-layout>
    ...
    <div class="mt-16"> <!-- Aumentar el margen superior -->
        ...
        <div class="flex justify-end items-center mb-4"> <!-- Mover el botón a la derecha -->
            <button id="inscribirBtn" class="bg-[#1c313b] text-white px-4 py-2 rounded mr-12">+ Inscribir mi perro</button>
        </div>
        <div class="overflow-x-auto w-11/12 mx-auto mb-10">
            <table class="min-w-full bg-white border border-gray-200">                <thead>
                    <tr>
                ...
                <tbody>
                    @foreach($pruebas as $prueba)
                        <tr class="border-b border-gray-200">
                            <td class="px-4 py-2 border-r border-gray-200 text-center" style="max-width: 200px;">
                            ...
                            <td class="px-4 py-2 border-r border-gray-200 text-center">{{ $prueba->disciplina }}</td>
                            <td class="px-4 py-2 border-r border-gray-200 text-center">{{ str_replace('|', ' - ', $prueba->fecha) }}</td>                            
                            <td class="px-4 py-2 border-r border-gray-200 text-center">{{ $prueba->lugar }}</td>
                            <td class="px-4 py-2 border-r border-gray-200 text-center">{{ $prueba->nombre_juez_1 }}</td>
                            <td class="px-4 py-2 border-r border-gray-200 text-center">{{ $prueba->nombre_juez_2 }}</td>
                            <td class="px-4 py-2 text-center">{{ $prueba->nombre_juez_3 }}</td>                     
                        </tr>
                    @endforeach
                </tbody>
            </table>
...
<div id="modal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden z-50">
    ...
    function actualizarTotal() {
        ...
        document.querySelectorAll('.inscripcion').forEach(inscripcion => {
            ...
            const numeroSocio = "{{ Auth::user()->numero_socio }}";
            const esSocioValido = numerosSociosValidos.includes(numeroSocio);
    ...
    terminarInscripcionBtn.addEventListener('click', function () {
        const inscripciones = [];
        const numeroSocio = "{{ Auth::user()->numero_socio }}";
        const esSocioValido = numerosSociosValidos.includes(numeroSocio); // Definir esSocioValido aquí
        document.querySelectorAll('.inscripcion').forEach(inscripcion => {
